...
class ApiClient {
  ...
  private async request<T>(path: string, init: RequestInit, sendJsonHeader = true, raw = false): Promise<ApiResponse<T>> {
    ...
    if (raw) {
      ...
      // app's standard error envelope, so error parsing matches the normal
      // path. Same fix applied to web-provider's api-client.ts.
      if (!res.ok) {
...
// ---------------------------------------------------------------------------
// Earnings — real /providers/me/earnings/* endpoints. Raw (unwrapped)
// responses on success — see ApiClient.getRaw.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Performance — ranking_router.py (rank) + kpis_router.py (period KPI
// history + summary). Both raw (unwrapped) on success.
// ---------------------------------------------------------------------------
...
// customer star-rating domain. NOT the same as onboarding's reviews_router.py
// (job sign-off evidence, unrelated to star ratings — a deceptively similar
// name for a different backend feature). Read-only: no reply endpoint
// exists. No joined customer name field either, only a raw customer_id.
// ---------------------------------------------------------------------------
...
// envelope, so this uses the normal get/post/patch, not the raw variants.
// No hard delete or re-activate — only deactivate (ACTIVE -> INACTIVE, one-
// way). No jobs/rating fields on the real record; job assignment and
// equipment tracking are separate, unexplored backend domains — out of
// scope here, not fabricated.
// ---------------------------------------------------------------------------
...
// No hard delete, only deactivate (one-way, like Team).
// ---------------------------------------------------------------------------
