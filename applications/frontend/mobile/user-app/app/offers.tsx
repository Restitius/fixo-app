...
function discountAmount(p: Promotion) {
...
export default function SpecialOffers() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      <ScreenHeader title={t('offers.title')} back="/(tabs)/home" />
      <ScrollView contentContainerStyle={{ paddingBottom: 32 }}>
        <View className="flex-col gap-4 px-6 mt-2">
          {offers === null ? (
            <View className="h-32 rounded-3xl bg-[#f5f5f5]" />
          ) : offers.length === 0 ? (
            <Text className="text-center text-muted py-8 text-[14px]">{t('offers.empty')}</Text>
          ) : (
            offers.map((o, i) => (
              <View key={o.promo_id} className="rounded-3xl p-6" style={{ backgroundColor: COLOR_PALETTE[i % COLOR_PALETTE.length] }}>
                <View className="flex-row items-center justify-between">
                  <View className="flex-row items-center gap-1 bg-white/20 rounded-full px-3 py-1 self-start">
                    ...
                    <Text className="text-[12px] font-bold text-white">{t('offers.discountOff', { amount: discountAmount(o) })}</Text>
                  </View>
                </View>
                ...
                <Text className="text-[12px] text-white/75 mt-4">{t('offers.codeValidUntil', { code: o.code, date: fmtDate(o.valid_until) })}</Text>
              </View>
            ))
          )}
        </View>
