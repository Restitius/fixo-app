...
const resources = {
  en: { auth: enAuth, profile: enProfile, tabs: enTabs, misc: enMisc, booking: enBooking, services: enServices },
  sw: { auth: swAuth, profile: swProfile, tabs: swTabs, misc: swMisc, booking: swBooking, services: swServices },
  fr: { auth: frAuth, profile: frProfile, tabs: frTabs, misc: frMisc, booking: frBooking, services: frServices },
  ar: { auth: arAuth, profile: arProfile, tabs: arTabs, misc: arMisc, booking: arBooking, services: arServices },
  pt: { auth: ptAuth, profile: ptProfile, tabs: ptTabs, misc: ptMisc, booking: ptBooking, services: ptServices },
  es: { auth: esAuth, profile: esProfile, tabs: esTabs, misc: esMisc, booking: esBooking, services: esServices },
  de: { auth: deAuth, profile: deProfile, tabs: deTabs, misc: deMisc, booking: deBooking, services: deServices },
  zh: { auth: zhAuth, profile: zhProfile, tabs: zhTabs, misc: zhMisc, booking: zhBooking, services: zhServices },
  hi: { auth: hiAuth, profile: hiProfile, tabs: hiTabs, misc: hiMisc, booking: hiBooking, services: hiServices },
  ja: { auth: jaAuth, profile: jaProfile, tabs: jaTabs, misc: jaMisc, booking: jaBooking, services: jaServices },
}
...
i18next.use(initReactI18next).init({
  ...
  lng: 'en',
  fallbackLng: 'en',
  ns: ['auth', 'profile', 'tabs', 'misc', 'booking', 'services'],
  defaultNS: 'auth',
