-- PRV.REQUESTS.GET — one incoming request's full detail (owner view)
SELECT mc.match_id, mc.score, mc.rank_pos, mc.strategy,
       ...
       prop.bedrooms, prop.bathrooms, prop.notes AS property_notes,
 ...
 WHERE mc.provider_id = CAST(:user_id AS uuid)
   AND r.request_id = CAST(:request_id AS uuid);