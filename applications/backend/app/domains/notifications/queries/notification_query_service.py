"""NotificationQueryService — governed persistence for notifications (no SQL here).

STATUS: NOTIFICATION.* statements are not yet present in app/queries/registry.yaml.
...
class NotificationQueryService:
    ...
    async def find(self, user_id: str, entity_id: int) -> Any:
        raise NotImplementedFeatureError(
            "Governed query NOTIFICATION.GET_BY_ID not registered yet - "
            "add SQL + registry.yaml entry first"
        )

    async def list(self, filters: dict[str, Any]) -> Any:
        raise NotImplementedFeatureError(
            "Governed query NOTIFICATION.LIST not registered yet - "
            "add SQL + registry.yaml entry first"
        )
