"""UserQueryService — governed persistence for users (no SQL here).

STATUS: USER.* statements are not yet present in app/queries/registry.yaml.
...
class UserQueryService:
    ...
    async def find(self, user_id: str, entity_id: int) -> Any:
        raise NotImplementedFeatureError(
            "Governed query USER.GET_BY_ID not registered yet - "
            "add SQL + registry.yaml entry first"
        )

    async def list(self, filters: dict[str, Any]) -> Any:
        raise NotImplementedFeatureError(
            "Governed query USER.LIST not registered yet - "
            "add SQL + registry.yaml entry first"
        )
