"""SupportService - business rules for support (ports only)."""
from __future__ import annotations

from typing import Any

from app.ports.persistence.support_repository import SupportRepositoryPort
from app.shared.protection_errors import TicketClosed, TicketNotFound

_CATEGORIES = {"GENERAL", "BILLING", "ACCOUNT", "TECHNICAL", "BOOKING", "OTHER"}
_PRIORITIES = {"LOW", "MEDIUM", "HIGH", "URGENT"}
_SENDERS = {"CUSTOMER", "SUPPORT"}


class SupportService:
    """Bridges customer intents to the support repository port.

    The service only touches business concepts (ticket, message, priority);
    the query IDs live in the adapter layer below.
    """

    def __init__(self, repository: SupportRepositoryPort) -> None:
    ...
    async def list_tickets(
        ...
        return await self._repo.list_tickets(customer_id, limit=limit, offset=offset)

    async def get_ticket(self, ticket_id: str, customer_id: str) -> dict[str, Any]:
        ticket = await self._repo.get_ticket(ticket_id=ticket_id, customer_id=customer_id)
        if ticket is None:
    ...
    async def add_message(
        ...
        await self.get_ticket(ticket_id, customer_id)
        ...
        message = await self._repo.add_message(
            ticket_id=ticket_id, customer_id=customer_id, sender=sender, body=body
        )
        if message is None:
            raise TicketClosed("Ticket is closed and no longer accepts messages")
        return message
    ...
    async def list_messages(
        ...
        return await self._repo.list_messages(
            ticket_id=ticket_id, customer_id=customer_id, limit=limit, offset=offset
        )
