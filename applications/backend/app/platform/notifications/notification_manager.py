...
class NotificationManager:
    ...
    async def notify(
        self, customer_id: str, *, ntype: str | None = None, title: str | None = None,
        ...
        await self._sql.execute(
            ...
            {"customer_id": customer_id, "type": ntype,
             "title": (title or "")[:160], "body": (body or "")[:500] or None,
        ...
        for row in claimed or []:
            try:
                ...
                await self._sql.execute(
                    ...
                    {"outbox_id": row["outbox_id"]}, fetch="one",
                )
                processed += 1
                ...
                failed += 1
        return {"claimed": len(claimed or []), "processed": processed, "failed": failed}
    ...
    async def _process_one(self, row: dict[str, Any]) -> None:
        ...
        for channel in definition.channels_for(recipient_type):
            if channel == "database":
                await self._write_inbox(recipient_type, recipient_id, key, definition, title, body, payload)
            elif channel == "sms":
                await self._deliver_channel(
                    outbox_id, "sms", recipient_type, recipient_id, definition,
                    lambda: self._send_sms(outbox_id, contact.get("phone"), title, body),
                )
            elif channel == "email":
                await self._deliver_channel(
                    outbox_id, "email", recipient_type, recipient_id, definition,
                    lambda: self._send_email(contact.get("email"), title, body),
                )

    async def _deliver_channel(
        self, outbox_id: str, channel: str, recipient_type: str, recipient_id: str,
        definition: Any, send: Any,
    ) -> None:
        ...
        attempt = await self._sql.execute(
            ...
            {
                "outbox_id": outbox_id, "channel": channel, "provider_reference": None,
                "status": "pending", "attempt_no": 1, "next_retry_at": None, "failure_reason": None,
            },
            fetch="one",
        )
        attempt_id = str(attempt["attempt_id"])
        try:
            provider_reference = await send()
            await self._sql.execute(
                NotificationQueryIds.DELIVERY_UPDATE_STATUS,
                {
                    "attempt_id": attempt_id, "status": "delivered", "failure_reason": None,
                    "next_retry_at": None, "provider_reference": provider_reference,
                },
                fetch="one",
            )
            ...
            await self._sql.execute(
                NotificationQueryIds.DELIVERY_UPDATE_STATUS,
                {
                    ...
                    "failure_reason": str(exc)[:500], "next_retry_at": next_retry_at,
    ...
    async def _send_sms(self, outbox_id: str, phone: str | None, title: str, body: str) -> str | None:
        ...
        result = await self._messaging.send_sms(
            phone, f"{title}: {body}"[:480], notification_id=outbox_id,
        )
    ...
    async def _write_inbox(
        self, recipient_type: str, recipient_id: str, key: str, definition: Any,
        title: str, body: str, payload: dict[str, Any],
    ) -> None:
        ref_type, ref_id = _derive_reference(payload)
        if recipient_type == "customer":
            await self._sql.execute(
                NotificationQueryIds.CREATE,
                {"customer_id": recipient_id, "type": key,
            ...
            await self._sql.execute(
                ...
                {"user_id": recipient_id, "channel": "in_app", "category": definition.category,
                 "title": title[:256], "body": body, "reference_type": ref_type, "reference_id": ref_id},
                fetch="one",
            )
    ...
    async def _recipient_contact(self, recipient_type: str, recipient_id: str) -> dict[str, Any]:
        query_id = (
            NotificationQueryIds.CUSTOMER_LANGUAGE_GET if recipient_type == "customer"
    ...
    async def _preference_allows(
        ...
        query_id = (
            NotificationQueryIds.CUSTOMER_PREFERENCE_LIST if recipient_type == "customer"
